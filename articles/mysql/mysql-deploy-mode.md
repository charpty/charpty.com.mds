...
数据分布在各个库中，要路由到具体库，此时需要选择一个分库字段，一般是业务结构的某个种类ID，每次做查询时只要把这个分库ID带上就行。但是总有业务查询就是要去查这个分库ID，这个时候就需要一些技巧来处理了。

可以把用来查ID的字段的信息就带在分库ID之中，当然这种方法只能带上一个字段，根本上也就是把这个字段也当作了分库的依据。

更加通俗的做法是建立mapper表，把该字段与分库ID建立关系，这样可以先通过一次查数据（或缓存）获取分库ID，然后再定位到具体数据库实例查数据。mapper表可以在每个库都存放，这样也可以直接关联。

分库之后还有个问题就是跨库事务，```mysql```支持XA事务，但是通常情况下借助于成熟的中间件是更好的选择。
